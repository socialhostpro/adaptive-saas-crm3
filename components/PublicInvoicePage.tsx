        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-950">
                <div className="text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mt-2">The invoice you are looking for does not exist or could not be loaded.</p>
                </div>
            </div>
        );
    }
                ...
                <div className="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8 bg-white dark:bg-gray-900 shadow-lg rounded-lg">
                    <div className="flex flex-col sm:flex-row justify-between items-start mb-8">
                        <div>
                            ...
                            <p className="text-gray-500 dark:text-gray-400 mt-1">{invoice.invoiceNumber}</p>
                            <div className="mt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
                        <div>
                            <h4 className="text-sm font-semibold uppercase text-gray-400 dark:text-gray-500 mb-2">Bill To</h4>
                        ...
                        <div className="text-left sm:text-right">
                            <dl className="grid grid-cols-2 gap-x-4">
                                ...
                                <dd className="text-sm text-gray-800 dark:text-gray-200">{new Date(invoice.issueDate).toLocaleDateString()}</dd>
                                <dt className="text-sm font-semibold text-gray-500 dark:text-gray-400">Due Date:</dt>
                                <dd className="text-sm text-gray-800 dark:text-gray-200">{new Date(invoice.dueDate).toLocaleDateString()}</dd>
                            </dl>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end">
                        <div className="w-full max-w-sm space-y-2">
                            <dl className="flex justify-between text-sm">
                                ...
                                <dd className="text-gray-800 dark:text-gray-200">${invoice.totalAmount.toFixed(2)}</dd>
                            </dl>
                            <dl className="flex justify-between text-sm">
                                <dt className="text-gray-500 dark:text-gray-400">Amount Paid:</dt>
                                <dd className="text-gray-800 dark:text-gray-200">-${paidAmount.toFixed(2)}</dd>
                            </dl>
                            <dl className="flex justify-between font-bold text-lg border-t border-gray-200 dark:border-gray-700 pt-2 mt-2">
                                <dt className="text-gray-900 dark:text-white">Amount Due:</dt>
                                <dd className="text-primary-600 dark:text-primary-500">${amountDue.toFixed(2)}</dd>
                            </dl>
                        </div>
                    </div>
                    
                    <div className="mt-10 flex justify-end gap-3 no-print">
                        <button onClick={() => window.print()} className="bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 font-semibold px-4 py-2 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">Download PDF</button>
                        {amountDue > 0 && <button className="bg-primary-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-primary-700 transition-colors">Pay Now</button>}
                    </div>

import React, { useState } from 'react';
import { EmailTemplate } from '../../types';
import EditEmailTemplateModal from '../company/EditEmailTemplateModal';
import { supabase } from '../../lib/supabaseClient';

interface SaaSEmailManagementProps {
    templates: EmailTemplate[];
    setTemplates: React.Dispatch<React.SetStateAction<EmailTemplate[]>>;
}

const SaaSEmailManagement: React.FC<SaaSEmailManagementProps> = ({ templates, setTemplates }) => {
    const [isEditModalOpen, setEditModalOpen] = useState(false);
    const [selectedTemplate, setSelectedTemplate] = useState<EmailTemplate | null>(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const handleToggle = async (id: string, isEnabled: boolean) => {
        setLoading(true);
        setError(null);
        const { data, error: updateError } = await supabase
            .from('email_templates')
            .update({ is_enabled: isEnabled })
            .eq('id', id)
            .select();
        if (updateError) {
            setError('Failed to update template status');
        } else if (data && data[0]) {
            setTemplates(prev => prev.map(t => (t.id === id ? { ...t, isEnabled: data[0].is_enabled } : t)));
        }
        setLoading(false);
    };

    const handleEditClick = (template: EmailTemplate) => {
        setSelectedTemplate(template);
        setEditModalOpen(true);
    };

    const handleSaveChanges = async (updatedTemplate: EmailTemplate) => {
        setLoading(true);
        setError(null);
        const { data, error: updateError } = await supabase
            .from('email_templates')
            .update({
                subject: updatedTemplate.subject,
                body: updatedTemplate.body
            })
            .eq('id', updatedTemplate.id)
            .select();
        if (updateError) {
            setError('Failed to update template');
        } else if (data && data[0]) {
            setTemplates(prev => prev.map(t => (t.id === updatedTemplate.id ? { ...t, ...data[0] } : t)));
            setEditModalOpen(false);
        }
        setLoading(false);
    };

    return (
        <>
            {loading && <div className="text-center py-2 text-sm text-gray-500">Saving...</div>}
            {error && <div className="text-center py-2 text-sm text-red-500">{error}</div>}
            <div className="space-y-6">
                <div>
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">SaaS Email Templates</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage automated emails sent by the platform to your customers.</p>
                </div>

                <div className="space-y-4">
                    {templates.map(template => (
                        <div key={template.id} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg flex flex-col md:flex-row justify-between md:items-center gap-4">
                            <div className="flex-grow">
                                <div className="flex items-center gap-3">
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                            checked={template.isEnabled}
                                            onChange={(e) => handleToggle(template.id, e.target.checked)}
                                            className="sr-only peer"
                                            disabled={loading}
                                        />
                                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-500 peer-checked:bg-primary-600"></div>
                                    </label>
                                    <h3 className="font-semibold text-gray-900 dark:text-white">{template.name}</h3>
                                </div>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 md:ml-14">{template.description}</p>
                            </div>
                            <div className="flex-shrink-0 self-end md:self-center">
                                <button onClick={() => handleEditClick(template)} className="text-sm font-semibold text-primary-600 dark:text-primary-400 hover:underline" disabled={loading}>
                                    Edit Template
                                </button>
                            </div>
                        </div>
                    ))}
                </div>
            </div>
            {selectedTemplate && (
                <EditEmailTemplateModal
                    isOpen={isEditModalOpen}
                    onClose={() => setEditModalOpen(false)}
                    template={selectedTemplate}
                    onSave={handleSaveChanges}
                />
            )}
        </>
    );
};

export default SaaSEmailManagement;

...
import { Contact, LineItem, Estimate, EstimateStatus, Product } from '../types';

interface CreateEstimateModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (estimate: Omit<Estimate, 'id' | 'estimateNumber'>) => void;
    contacts: Contact[];
    products: Product[];
}
...
const CreateEstimateModal: React.FC<CreateEstimateModalProps> = ({ isOpen, onClose, onSubmit, contacts, products }) => {
    ...
    const handleSubmit = (status: EstimateStatus) => {
        if (!formData.contactId || !formData.issueDate || !formData.expiryDate || formData.lineItems.some(i => !i.description || i.quantity <= 0)) {
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white">Create New Estimate</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="md:col-span-1">
                         ...
                         <div className="md:col-span-1">
                            ...
                            <input type="date" id="expiryDate" name="expiryDate" value={formData.expiryDate} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-sm focus:ring-primary-500 focus:border-primary-500"/>
                        </div>
                    </div>
                ...
                <footer className="flex-shrink-0 p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex gap-3">
                        <button onClick={() => handleSubmit(EstimateStatus.Draft)} className="bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 font-semibold px-4 py-2 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">Save as Draft</button>
                        <button onClick={() => handleSubmit(EstimateStatus.Sent)} className="bg-primary-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-primary-700 transition-colors">Create & Send Estimate</button>
                    </div>
                </footer>
            </div>
        </div>
